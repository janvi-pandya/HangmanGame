...
func main() {
	...
	//generate a random number from 4 to 15 and start the first round 
	reInitRandomNumber(&numberOfLetters)
...
func playHangman(numberOfLetters int) (playAgain string, isWinner bool) {
	...
	for {
		fmt.Println("Select the game type:")
        fmt.Println("[e] Easy - use simple 4,5 or 6 letter words only")
        fmt.Println("[h] Hard - use difficult 4 to 15 letter words")
        fmt.Scanln(&gameType)
		...
		if gameType == "e" || gameType == "h" {
			...
			fmt.Println("Please enter [e] or [h] key")
		}
	}
...
// to reinitialize a psuedo random number between 4 and 15
func reInitRandomNumber(toInit *int) {
...
func printScore(wins, loses int) {
    ...
    fmt.Printf("You have %d wins and %d loses\n", wins, loses)
	fmt.Printf("################################\n")
	return
}
